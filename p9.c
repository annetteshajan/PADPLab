...
int main ( int argc, char *argv[] ); void ring_io ( int p, int id );
int main ( int argc, char *argv[] ) {
int error; int id; int p;
/* Initialize MPI.*/ MPI_Init ( &argc, &argv );
/*Get the number of processes.*/
MPI_Comm_size ( MPI_COMM_WORLD, &p );
/*Get the individual process ID.*/
MPI_Comm_rank ( MPI_COMM_WORLD, &id );
/*Print a message.*/ if ( id == 0 )
{
printf ( "\n" );
printf ( "RING_MPI:\n" );
...
int j;
int n;
int n_test[5] = { 100, 1000, 10000, 100000, 1000000 }; int n_test_num = 5;
int source;
MPI_Status status;
double tave;
int test;
...
MPI_Recv ( x, n, MPI_DOUBLE, source, 0, MPI_COMM_WORLD, &status ); wtime = MPI_Wtime ( ) - wtime;
/* Record the time it took.*/ tave = tave + wtime;
if ( wtime < tmin )
{
tmin = wtime; }
if ( tmax < wtime ) {
...
/* Worker ID must receive first from ID-1, then send to ID+1.*/ else
{
source = id - 1;
dest = ( ( id + 1 ) % p );
for ( test = 1; test <= test_num; test++ ) {
